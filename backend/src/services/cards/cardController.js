...
/**
 * Confirma o pagamento e ativa a cartela para o jogo
 * Ação exclusiva do GESTOR (ADM)
 */
async function handleConfirmarPagamento(req, res) {
    const { cartelaId, gestorId } = req.body;

    if (!cartelaId || !gestorId) {
        return res.status(400).json({ error: "cartelaId e gestorId são obrigatórios." });
    }

    try {
        // 1. Atualiza a cartela apenas se ela estiver RESERVADA
        const updateResult = await db.query(
            `UPDATE Cartelas 
             SET status = 'PAGA_ATIVA', 
                 data_pagamento = NOW() 
             WHERE id_cartela = $1 AND status = 'RESERVADA'
             RETURNING id_jogo, id_dono`,
            [cartelaId]
        );

        if (updateResult.rows.length === 0) {
            return res.status(400).json({ 
                error: "Cartela não encontrada ou não está em estado de reserva pendente." 
            });
        }

        const { id_jogo, id_dono } = updateResult.rows[0];

        // 2. Registra a auditoria do pagamento
        await db.query(
            `INSERT INTO Logs (id_jogo, id_usuario, tipo_evento, dados_evento) 
             VALUES ($1, $2, 'PAGAMENTO', $3)`,
            [id_jogo, gestorId, JSON.stringify({ cartelaId, clienteId: id_dono })]
        );

        // 3. Resposta de sucesso
        // TODO: Aqui dispararemos o evento para o MESSAGING enviar a cartela ao cliente
        return res.status(200).json({ 
            message: "Pagamento confirmado. Cartela ativada com sucesso!",
            cartelaId 
        });
